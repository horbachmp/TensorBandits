...
class TensorElimination:
    def __init__(self, dimensions, ranks, bandit, total_steps=20000, explore_steps=10, lambda1=0.01, lambda2=20.0, conf_int_len=0.3, img_name=None, update_arm_on_step=None, delete_arm_on_step=None) -> None:
        self.bandit  = bandit
        ...
        self.Reward_vec_est = np.zeros(self.dimensions, dtype='float16')
        self.Reward_vec_sum = np.zeros(self.dimensions, dtype='float16')
        self.Reward_vec_est_UUT = np.zeros(self.dimensions, dtype='float16')
        self.have_info = np.zeros(self.dimensions).astype(bool)
        self.num_pulls = np.zeros(self.dimensions, dtype='int')
    ...
    def CreateArmTensorByIndex(self, ind, vectorise=True):
        ...
        for i in range(len(ind)):
            vec_e = np.zeros(self.dimensions[i], dtype='float16')
            vec_e[ind[i]] = 1
...
def main():
    ...
    bandit = OpenBanditTest()
    algo = TensorElimination(dimensions=[30,30,30], ranks=[3,3,3], bandit=bandit)
    algo.PlayAlgo()
